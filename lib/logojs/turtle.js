...
(function(global) {
  ...
  function CanvasTurtle(canvas_ctx, w, h, events, options) {
    ...
    this.turtle_element = options.turtleElement;
    this.turtle_size = options.turtleSize;
    this.width = Number(w);
  ...
  Object.defineProperties(CanvasTurtle.prototype, {
    ...
    _tick: {
      value: function() {
        ...
        if (this.visible) {
          this.turtle_element.style.display = "block";
          this.turtle_element.style.left =
            this.width / 2 + this.x * this.sx - this.turtle_size / 2 + "px";
          this.turtle_element.style.top =
            this.height / 2 - this.y * this.sy - this.turtle_size / 2 + "px";
          this.turtle_element.style.width = this.turtle_size + "px";
          this.turtle_element.style.height = this.turtle_size + "px";
          this.turtle_element.style.transform = `rotate(${-this.r + 1.5708}rad)`;
        } else {
          this.turtle_element.style.display = "none";
        }
      }
    },

    _moveto: {
      value: function(x, y, setpos) {
        ...
        var w = this.width / this.sx,
          h = this.height / this.sy;
    ...
    fillpath: {
      value: function(fillcolor) {
        ...
        if (this.filling === 0) {
          ...
          if (this.pendown) this.canvas_ctx.stroke();
    ...
    arc: {
      value: function(angle, radius) {
        if (this.turtlemode == "wrap") {
          [this.x, this.x + this.width / this.sx, this.x - this.width / this.sx].forEach(
            function(x) {
              [this.y, this.y + this.height / this.sy, this.y - this.height / this.sy].forEach(
                function(y) {
                  if (!this.filling) this.canvas_ctx.beginPath();
                  this.canvas_ctx.arc(x, y, radius, this.r, this.r - deg2rad(angle), angle > 0);
                  if (!this.filling) this.canvas_ctx.stroke();
                }.bind(this)
              );
            }.bind(this)
          );
    ...
    penmode: {
      ...
      set: function(penmode) {
        ...
        this.canvas_ctx.globalCompositeOperation =
          this.penmode === "erase"
            ? "destination-out"
            : this.penmode === "reverse"
            ? "difference"
            : "source-over";
        if (penmode === "paint")
          this.canvas_ctx.strokeStyle = this.canvas_ctx.fillStyle = this.color;
        else this.canvas_ctx.strokeStyle = this.canvas_ctx.fillStyle = "#ffffff";
      }
    },

    turtlemode: {
      set: function(turtlemode) {
        this._turtlemode = turtlemode;
      },
      get: function() {
        return this._turtlemode;
      }
    },
    ...
    clickpos: {
      get: function() {
        return [this._clickx, this._clicky];
      }
    },
