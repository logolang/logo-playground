...
function LogoInterpreter(turtle, stream, savehook) {
  ...
  this.localize = null;
  function __(string) {
    if (self.localize) return self.localize(string) || string;
    return string;
  }
  ...
  function isKeyword(atom, match) {
    if (Type(atom) !== "word") return false;
    atom = String(atom).toUpperCase();
    if (self.keywordAlias) atom = self.keywordAlias(atom) || atom;
    return atom === match;
  }
  ...
  function serialExecute(funcs) {
    ...
    return promiseLoop(function(loop, resolve, reject) {
      ...
      Promise.resolve(funcs.shift()()).then(function(result) {
        results.push(result);
        loop();
      }, reject);
    });
  ...
  function promiseFinally(promise, finalBlock) {
    return promise.then(
      function(result) {
        return Promise.resolve(finalBlock()).then(function() {
          return result;
        });
      },
      function(err) {
  ...
  function to_arity($$func$$, arity) {
    ...
    if ($$func$$.length === arity) return $$func$$;

    for (var i = 0; i < arity; ++i) parms.push("a" + i);

    var f = eval(
      "(function " + $$func$$.name + "(" + parms.join(",") + ")" + "{ return $$func$$.apply(this, arguments); })"
    );
    return f;
  }
  ...
  function LogoArray(size, origin) {
    ...
    for (var i = 0; i < this._array.length; ++i) this._array[i] = [];
    this._origin = origin;
  }
  ...
  Object.defineProperties(LogoArray.prototype, {
    item: {
      value: function(i) {
        i = Number(i) | 0;
        i -= this._origin;
        if (i < 0 || i >= this._array.length) throw err("{_PROC_}: Index out of bounds", ERRORS.BAD_INPUT);
  ...
  Object.defineProperties(Stream.prototype, {
    ...
    peek: {
      value: function() {
        ...
        if (c === "\\") c += this._string.charAt(this._index + 1);
        return c;
      }
    },
    get: {
    ...
    _next: {
      value: function() {
        var c = this._string.charAt(this._index++);
        if (c === "\\") c += this._string.charAt(this._index++);
        return c;
      }
    },
    _skip: {
  ...
  function parse(string) {
    ...
    while (stream.peek()) {
      ...
      while (isWS(stream.peek())) stream.get();
  ...
  function parseOperator(stream) {
    var word = "";
    if (inChars(stream.peek(), OPERATOR_CHARS)) word += stream.get();
    if (
      (word === "<" && stream.peek() === "=") ||
      (word === ">" && stream.peek() === "=") ||
      (word === "<" && stream.peek() === ">")
  ...
  function parseNumber(stream) {
    var word = "";
    while (inRange(stream.peek(), "0", "9")) word += stream.get();
    if (stream.peek() === ".") word += stream.get();
    if (inRange(stream.peek(), "0", "9")) {
      while (inRange(stream.peek(), "0", "9")) word += stream.get();
    }
    if (stream.peek() === "E" || stream.peek() === "e") {
      word += stream.get();
      if (stream.peek() === "-" || stream.peek() === "+") word += stream.get();
      while (inRange(stream.peek(), "0", "9")) word += stream.get();
    }
    return word;
  }
  ...
  function parseList(stream) {
    ...
    while (true) {
      ...
      if (!c) throw err("Expected ']'", ERRORS.BAD_BRACKET);
      if (isWS(c)) continue;
      if (c === "]") return list;
      if (c === "[") {
      ...
      if (c === "}") throw err("Unexpected '}'", ERRORS.BAD_BRACE);
  ...
  function parseArray(stream) {
    ...
    while (true) {
      ...
      if (!c) throw err("Expected '}'", ERRORS.BAD_BRACE);
      if (isWS(c)) continue;
      if (c === "}") {
        while (isWS(stream.peek())) stream.get();
        if (stream.peek() === "@") {
      ...
      if (c === "]") throw err("Unexpected ']'", ERRORS.BAD_BRACKET);
      if (c === "{") {
        list.push(parseArray(stream));
        continue;
      }
    ...
    while (peek(list, ["=", "<", ">", "<=", ">=", "<>"])) {
      ...
      lhs = (function(lhs) {
        ...
        switch (op) {
          ...
          default:
            throw new Error("Internal error in expression parser");
        }
      })(lhs);
    }
  ...
  function defer(func /*, input...*/) {
    ...
    return function() {
      return serialExecute(input.slice()).then(function(args) {
  ...
  function finalExpression(list) {
    if (!list.length) throw err("Unexpected end of instructions", ERRORS.MISSING_PAREN);
    ...
    switch (Type(atom)) {
      ...
      case "word":
        ...
        if (atom === "(") {
          ...
          if (!list.length) throw err("Expected ')'", ERRORS.MISSING_PAREN);
          if (!peek(list, [")"])) throw err("Expected ')', saw {word}", { word: list.shift() }, ERRORS.MISSING_PAREN);
          list.shift();
          return result;
        }
        if (atom === ")") throw err("Unexpected ')'", ERRORS.BAD_PAREN);
        // Procedure dispatch
  ...
  self.dispatch = function(name, tokenlist, natural) {
    ...
    if (procedure.noeval) {
      return function() {
        self.stack.push(name);
        return promiseFinally(procedure.apply(self, args), function() {
          self.stack.pop();
        });
      };
    }
    ...
    return function() {
      ...
      return promiseFinally(
        serialExecute(args.slice()).then(function(args) {
  ...
  function aexpr(atom) {
    ...
    switch (Type(atom)) {
      case "word":
        if (isNumber(atom)) return parseFloat(atom);
        break;
    }
    throw err("Expected number", ERRORS.BAD_INPUT);
  }
  ...
  // use sifw().
  function lexpr(atom) {
    if (atom === undefined) throw err("{_PROC_}: Expected list", ERRORS.BAD_INPUT);
    switch (Type(atom)) {
      case "word":
  ...
  self.execute = function(statements, options) {
    ...
    return promiseLoop(function(loop, resolve, reject) {
      ...
      Promise.resolve(evaluateExpression(statements)).then(function(result) {
  ...
  self.run = function(string, options) {
    ...
    return self.queueTask(function() {
      ...
      // And execute it!
      return self.execute(atoms, options).catch(function(err) {
        if (!(err instanceof Bye)) throw err;
      });
    });
  };
  ...
  self.definition = function(name, proc) {
    function defn(atom) {
      switch (Type(atom)) {
        ...
        case "array":
          return "{ " + atom.list.map(defn).join(" ") + " }" + (atom.origin === 1 ? "" : "@" + atom.origin);
        default:
    ...
    def += proc.inputs
      ...
      .join("");
    def += proc.optional_inputs
      .map(function(op) {
        return " [:" + op[0] + " " + op[1].map(defn).join(" ") + "]";
      })
      .join("");
    if (proc.rest) def += " [:" + proc.rest + "]";
    if (proc.def !== undefined) def += " " + proc.def;
    ...
    def +=
      ...
      proc.block
        .map(defn)
        .join(" ")
        .replace(new RegExp(UNARY_MINUS + " ", "g"), "-");
    def += "\n" + "end";
  ...
  function stringify(thing) {
    switch (Type(thing)) {
      ...
      case "array":
        return "{" + thing.list.map(stringify).join(" ") + "}" + (thing.origin === 1 ? "" : "@" + thing.origin);
      default:
        return sexpr(thing);
    }
  }
  ...
  def(
    ...
    function(list) {
      ...
      if (isNumber(name) || isOperator(name)) throw err("TO: Expected identifier", ERRORS.BAD_INPUT);
      ...
      while (list.length) {
        ...
        if (state === OPTIONAL) {
          if (Type(atom) === "list" && atom.length > 1 && String(atom[0]).charAt(0) === ":") {
      ...
      if (!sawEnd) throw err("TO: Expected END", ERRORS.BAD_INPUT);
  ...
  function defineProc(name, inputs, optional_inputs, rest, def, block) {
    ...
    if (def !== undefined && (def < inputs.length || (!rest && def > inputs.length + optional_inputs.length))) {
    ...
    var func = function() {
      ...
      var i = 0,
        op;
      for (; i < inputs.length && i < arguments.length; ++i) scope.set(inputs[i], { value: arguments[i] });
      ...
      if (rest) scope.set(rest, { value: [].slice.call(arguments, i) });

      return promiseFinally(
        self.execute(block).then(promiseYield, function(err) {
          if (err instanceof Output) return err.output;
          throw err;
        }),
        function() {
          self.scopes.pop();
    ...
    if (savehook) savehook(name, self.definition(name, proc));
  }

  def("def", function(list) {
    var name = sexpr(list);
    var proc = this.routines.get(name);
    if (!proc) throw err("{_PROC_}: Don't know how to {name:U}", { name: name }, ERRORS.BAD_PROC);
    if (!proc.inputs) {
  ...
  def(
    ...
    function(thing1, thing2) {
      return Array.from(arguments).map(function(x) {
        return x;
      }); // Make a copy
    },
    { minimum: 0, maximum: -1 }
  ...
  def(
    ...
    function(size) {
      size = aexpr(size);
      if (size < 1) throw err("{_PROC_}: Array size must be positive integer", ERRORS.BAD_INPUT);
  ...
  def(
    ...
    function(sizes) {
      ...
      function make(index) {
        var n = sizes[index],
          a = new LogoArray(n, origin);
        if (index + 1 < sizes.length) {
          for (var i = 0; i < n; ++i) a.setItem(i + origin, make(index + 1));
        }
        return a;
      }
  ...
  def(
    ...
    function(list) {
      list = lexpr(list);
      var origin = 1;
      if (arguments.length > 1) origin = aexpr(arguments[1]);
      return LogoArray.from(list, origin);
    },
    { maximum: 2 }
  );
  ...
  function item(index, thing) {
    switch (Type(thing)) {
      case "list":
        if (index < 1 || index > thing.length) throw err("{_PROC_}: Index out of bounds", ERRORS.BAD_INPUT);
  ...
  def("mditem", function(indexes, thing) {
    indexes = lexpr(indexes)
      .map(aexpr)
      .map(function(n) {
        return n | 0;
      });
    while (indexes.length) thing = item(indexes.shift(), thing);
    return thing;
  });
  ...
  def("remove", function(thing, list) {
    return sifw(
      list,
      lexpr(list).filter(function(x) {
        return !equal(x, thing);
      })
    );
  });
  ...
  def("quoted", function(thing) {
    if (Type(thing) === "word") return '"' + thing;
    return thing;
  });
  ...
  function contains(atom, value) {
    ...
    switch (Type(atom)) {
      case "list":
        return atom.some(function(a) {
          return contains(a, value);
        });
      case "array":
        return atom.list.some(function(a) {
          return contains(a, value);
        });
      default:
        return false;
  ...
  def("setitem", function(index, array, value) {
    index = aexpr(index);
    if (Type(array) !== "array") throw err("{_PROC_}: Expected array", ERRORS.BAD_INPUT);
    if (contains(value, array)) throw err("{_PROC_}: Can't create circular array", ERRORS.BAD_INPUT);
  ...
  def("mdsetitem", function(indexes, thing, value) {
    ...
    if (Type(thing) !== "array") throw err("{_PROC_}: Expected array", ERRORS.BAD_INPUT);
    if (contains(value, thing)) throw err("{_PROC_}: Can't create circular array", ERRORS.BAD_INPUT);
  ...
  def(".setfirst", function(list, value) {
    if (Type(list) !== "list") throw err("{_PROC_}: Expected list", ERRORS.BAD_INPUT);
    list[0] = value;
  });

  def(".setbf", function(list, value) {
    if (Type(list) !== "list") throw err("{_PROC_}: Expected non-empty list", ERRORS.BAD_INPUT);
    if (list.length < 1) throw err("{_PROC_}: Expected non-empty list", ERRORS.BAD_INPUT);
    value = lexpr(value);
    list.length = 1;
  ...
  def(["wordp", "word?"], function(thing) {
    return Type(thing) === "word" ? 1 : 0;
  });
  def(["listp", "list?"], function(thing) {
    return Type(thing) === "list" ? 1 : 0;
  });
  def(["arrayp", "array?"], function(thing) {
    return Type(thing) === "array" ? 1 : 0;
  });
  def(["numberp", "number?"], function(thing) {
    return Type(thing) === "word" && isNumber(thing) ? 1 : 0;
  });
  def(["numberwang"], function(thing) {
    return this.prng.next() < 0.5 ? 1 : 0;
  });

  def(["equalp", "equal?"], function(a, b) {
    return equal(a, b) ? 1 : 0;
  });
  def(["notequalp", "notequal?"], function(a, b) {
    return !equal(a, b) ? 1 : 0;
  });
  ...
  def(["emptyp", "empty?"], function(thing) {
    switch (Type(thing)) {
      case "word":
        return String(thing).length === 0 ? 1 : 0;
      case "list":
        return thing.length === 0 ? 1 : 0;
      default:
        return 0;
    }
  });
  ...
  def("count", function(thing) {
    if (Type(thing) === "array") return thing.length;
    return lexpr(thing).length;
  });
  def("ascii", function(chr) {
    return sexpr(chr).charCodeAt(0);
  });
  // Not Supported: rawascii
  def("char", function(integer) {
  ...
  def("lowercase", function(word) {
    return sexpr(word).toLowerCase();
  });
  def("uppercase", function(word) {
  ...
  def(
    "readlist",
    function() {
      return (arguments.length > 0 ? stream.read(stringify_nodecorate(arguments[0])) : stream.read()).then(function(
        word
      ) {
        return parse("[" + word + "]")[0];
      });
  ...
  def(
    "readword",
    function() {
      if (arguments.length > 0) return stream.read(stringify_nodecorate(arguments[0]));
  ...
  def(
    "quotient",
    function(a, b) {
      if (b !== undefined) return aexpr(a) / aexpr(b);
      else return 1 / aexpr(a);
    },
    { minimum: 1 }
  );
  ...
  def("radsin", function(a) {
    return Math.sin(aexpr(a));
  });
  def("radcos", function(a) {
    return Math.cos(aexpr(a));
  });
  def("radtan", function(a) {
  ...
  def(["greaterp", "greater?"], function(a, b) {
    return aexpr(a) > aexpr(b) ? 1 : 0;
  });
  def(["greaterequalp", "greaterequal?"], function(a, b) {
    return aexpr(a) >= aexpr(b) ? 1 : 0;
  });
  def(["lessp", "less?"], function(a, b) {
    return aexpr(a) < aexpr(b) ? 1 : 0;
  });
  def(["lessequalp", "lessequal?"], function(a, b) {
    return aexpr(a) <= aexpr(b) ? 1 : 0;
  });
  ...
  def("form", function(num, width, precision) {
    ...
    if (str.length < width) str = Array(1 + width - str.length).join(" ") + str;
    return str;
  });
  ...
  function booleanReduce(args, test, value) {
    return promiseLoop(function(loop, resolve, reject) {
      ...
      Promise.resolve(args.shift()()).then(function(result) {
        if (!test(result)) {
          resolve(result);
          return;
        }
  ...
  def(["forward", "fd"], function(a) {
    return turtle.move(aexpr(a));
  });
  def(["back", "bk"], function(a) {
    return turtle.move(-aexpr(a));
  });
  def(["left", "lt"], function(a) {
    return turtle.turn(-aexpr(a));
  });
  def(["right", "rt"], function(a) {
    return turtle.turn(aexpr(a));
  });

  // Left arrow:
  def(["\u2190"], function() {
    return turtle.turn(-15);
  });
  // Right arrow:
  def(["\u2192"], function() {
    return turtle.turn(15);
  ...
  def("setxy", function(x, y) {
    turtle.position = [aexpr(x), aexpr(y)];
  });
  def("setx", function(x) {
    turtle.position = [aexpr(x), undefined];
  });
  def("sety", function(y) {
    turtle.position = [undefined, aexpr(y)];
  });
  def(["setheading", "seth"], function(a) {
    turtle.heading = aexpr(a);
  });
  ...
  def("arc", function(angle, radius) {
    return turtle.arc(aexpr(angle), aexpr(radius));
  });
  ...
  def("xcor", function() {
    return turtle.position[0];
  });
  def("ycor", function() {
    return turtle.position[1];
  });
  ...
  def("scrunch", function() {
    return turtle.scrunch;
  });
  ...
  def(["showturtle", "st"], function() {
    turtle.visible = true;
  });
  def(["hideturtle", "ht"], function() {
    turtle.visible = false;
  });
  def("clean", function() {
    turtle.clear();
  });
  def(["clearscreen", "cs"], function() {
    turtle.clearscreen();
  });

  def("wrap", function() {
    turtle.turtlemode = "wrap";
  });
  def("window", function() {
    turtle.turtlemode = "window";
  });
  def("fence", function() {
    turtle.turtlemode = "fence";
  });
  ...
  def("filled", function(fillcolor, statements) {
    fillcolor = sexpr(parseColor(fillcolor));
  ...
  def("setlabelheight", function(a) {
    turtle.fontsize = aexpr(a);
  });

  def("setlabelfont", function(a) {
    turtle.fontname = sexpr(a);
  });
  ...
  //
  def(["pendown", "pd"], function() {
    turtle.pendown = true;
  });
  def(["penup", "pu"], function() {
    turtle.pendown = false;
  });

  def(["penpaint", "ppt"], function() {
    turtle.penmode = "paint";
  });
  def(["penerase", "pe"], function() {
    turtle.penmode = "erase";
  });
  def(["penreverse", "px"], function() {
    turtle.penmode = "reverse";
  });
  ...
  function parseColor(color) {
    ...
    if (self.colorAlias) return self.colorAlias(color) || color;
    return color;
  }
  ...
  def("setpalette", function(colornumber, color) {
    ...
    if (colornumber < 8) throw err("{_PROC_}: Expected number greater than 8", ERRORS.BAD_INPUT);
    PALETTE[colornumber] = parseColor(color);
  });

  def(["setpensize", "setwidth", "setpw"], function(a) {
    if (Type(a) === "list") turtle.penwidth = aexpr(a[0]);
    else turtle.penwidth = aexpr(a);
  });
  ...
  def("define", function(name, list) {
    name = sexpr(name);
    list = lexpr(list);
    if (list.length != 2) throw err("{_PROC_}: Expected list of length 2", ERRORS.BAD_INPUT);
  ...
  def("text", function(name) {
    ...
    if (proc.rest) inputs.push([proc.rest]);
    if (proc.def !== undefined) inputs.push(proc.def);
  ...
  def("gprop", function(plistname, propname) {
    ...
    propname = sexpr(propname);
    var plist = this.plists.get(plistname);
    if (!plist || !plist.has(propname)) return [];
  ...
  def(["primitivep", "primitive?"], function(name) {
    name = sexpr(name);
    return this.routines.has(name) && this.routines.get(name).primitive ? 1 : 0;
  });
  ...
  def("traced", function() {
    return [
      ...
      this.scopes.reduce(function(list, scope) {
        return list.concat(
          scope.keys().filter(function(x) {
            return scope.get(x).traced;
          })
        );
      }, []),
      this.plists.keys().filter(
        function(x) {
          return this.plists.get(x).traced;
        }.bind(this)
      )
    ];
  });
  ...
  def(["stepped"], function() {
    return [
      ...
      this.scopes.reduce(function(list, scope) {
        return list.concat(
          scope.keys().filter(function(x) {
            return scope.get(x).stepped;
          })
        );
      }, []),
      this.plists.keys().filter(
        function(x) {
          return this.plists.get(x).stepped;
        }.bind(this)
      )
    ];
  });
  ...
  def("procedures", function() {
    return this.routines.keys().filter(
      function(x) {
        return !this.routines.get(x).primitive && !this.routines.get(x).buried;
      }.bind(this)
    );
  ...
  def("namelist", function(varname) {
    if (Type(varname) === "list") varname = lexpr(varname);
    else varname = [sexpr(varname)];
    return [[], varname];
  });
  ...
  def("arity", function(name) {
    ...
    var proc = this.routines.get(name);
    if (!proc) throw err("{_PROC_}: Don't know how to {name:U}", { name: name }, ERRORS.BAD_PROC);
    if (proc.special) return [-1, -1, -1];

    return [proc.minimum, proc.default, proc.maximum];
  });
  ...
  def("ern", function(varname) {
    var varnamelist;
    if (Type(varname) === "list") varnamelist = lexpr(varname);
    else varnamelist = [sexpr(varname)];
  ...
  def("bury", function(list) {
    ...
    if (list.length) {
      ...
      procs.forEach(
        function(name) {
          name = sexpr(name);
          if (this.routines.has(name)) this.routines.get(name).buried = true;
        }.bind(this)
      );
    }
    ...
    if (list.length) {
      ...
      this.scopes.forEach(function(scope) {
        vars.forEach(function(name) {
          name = sexpr(name);
          if (scope.has(name)) scope.get(name).buried = true;
        });
      });
    }
    ...
    if (list.length) {
      ...
      plists.forEach(
        function(name) {
          name = sexpr(name);
          if (this.plists.has(name)) this.plists.get(name).buried = true;
        }.bind(this)
      );
    }
  });
  ...
  def("unbury", function(list) {
    ...
    if (list.length) {
      ...
      procs.forEach(
        function(name) {
          name = sexpr(name);
          if (this.routines.has(name)) this.routines.get(name).buried = false;
        }.bind(this)
      );
    }
    ...
    if (list.length) {
      ...
      this.scopes.forEach(function(scope) {
        vars.forEach(function(name) {
          name = sexpr(name);
          if (scope.has(name)) scope.get(name).buried = false;
        });
      });
    }
    ...
    if (list.length) {
      ...
      plists.forEach(
        function(name) {
          name = sexpr(name);
          if (this.plists.has(name)) this.plists.get(name).buried = false;
        }.bind(this)
      );
    }
  });
  ...
  def("runresult", function(statements) {
    statements = reparse(lexpr(statements));
    return this.execute(statements, { returnResult: true }).then(function(result) {
  ...
  def(
    "if",
    function(tf, statements) {
      if (Type(tf) === "list") tf = evaluateExpression(reparse(tf));

      var statements2 = arguments[2];

      return Promise.resolve(tf).then(
        function(tf) {
          tf = aexpr(tf);
          statements = reparse(lexpr(statements));
          if (!statements2) {
  ...
  def(["iftrue", "ift"], function(statements) {
    ...
    if (tf === undefined) throw err("{_PROC_}: Called without TEST", ERRORS.NO_TEST);
    return tf ? this.execute(statements, { returnResult: true }) : undefined;
  });
  ...
  def(
    "catch",
    function(tag, instructionlist) {
      tag = sexpr(tag).toUpperCase();
      instructionlist = reparse(lexpr(instructionlist));
      return this.execute(instructionlist, { returnResult: true }).catch(
        function(error) {
          if (!(error instanceof LogoError) || error.tag !== tag) throw error;
  ...
  def("error", function() {
    ...
    var list = [this.last_error.code, this.last_error.message, this.last_error.proc, this.last_error.line];
    this.last_error = undefined;
    return list;
  });
    ...
    return promiseLoop(
      function(loop, resolve, reject) {
        ...
        if (member === "," && list.length) {
          member = list.shift();
          if (Type(member) === "word") member = [member];
  ...
  def("for", function(control, statements) {
    ...
    return Promise.resolve(evaluateExpression(control))
      ...
      .then(function(r) {
        limit = aexpr(r);
        return control.length ? evaluateExpression(control) : limit < start ? -1 : 1;
      })
      .then(function(r) {
        step = aexpr(r);
      })
  ...
  def("case", function(value, clauses) {
    ...
    for (var i = 0; i < clauses.length; ++i) {
      ...
      if (isKeyword(first, "ELSE")) return evaluateExpression(clause);
      if (
        lexpr(first).some(function(x) {
  ...
  def("cond", function(clauses) {
    ...
    return promiseLoop(function(loop, resolve, reject) {
      ...
      evaluateExpression(reparse(lexpr(first))).then(function(result) {
        if (result) {
          resolve(evaluateExpression(clause));
          return;
        }
  ...
  def("apply", function(procname, list) {
    ...
    if (!routine) throw err("{_PROC_}: Don't know how to {name:U}", { name: procname }, ERRORS.BAD_PROC);
  ...
  def("foreach", function(procname, list) {
    ...
    return promiseLoop(function(loop, resolve, reject) {
      ...
      Promise.resolve(routine(list.shift())).then(loop, reject);
    });
  });

  def(
    "map",
    function(procname, list /*,  ... */) {
      procname = sexpr(procname);
      ...
      if (!lists.length) throw err("{_PROC_}: Expected list", ERRORS.BAD_INPUT);
      ...
      return promiseLoop(
        function(loop, resolve, reject) {
          ...
          var args = lists.map(function(l) {
            if (!l.length) throw err("{_PROC_}: Expected lists of equal length", ERRORS.BAD_INPUT);
            return l.shift();
          });
  ...
  def("find", function(procname, list) {
    ...
    return promiseLoop(function(loop, resolve, reject) {
      ...
      var item = list.shift();
      Promise.resolve(routine(item)).then(function(value) {
        if (value) {
          resolve(item);
          return;
        }
  ...
  def(
    ...
    function(procname, list) {
      ...
      if (!procedure) throw err("{_PROC_}: Don't know how to {name:U}", { name: procname }, ERRORS.BAD_PROC);
  ...
  def(
    ...
    function(procname, list /*,  ... */) {
      ...
      if (lists.length === 1) lists = lists[0].map(lexpr);
      ...
      return promiseLoop(
        function(loop, resolve, reject) {
          ...
          var args = indexes.map(function(v, i) {
            return lists[i][v];
          });
